...
const STOP_WORDS = new Set([
  "가이드", "가장", "가치", "각자", "강화", "건강한", "결과", "결정", "관련", "교훈", "구분", "구조", "규칙", "기록",
  "기본", "기억", "기억할", "기준", "놓치기", "다시", "다양한", "대상", "대응", "대한", "따져보는", "만드는", "먼저",
  "무엇", "무엇인", "방법", "방식", "바꾸는", "발생", "발생하", "배경", "변화", "보는", "본질", "비밀", "사례",
  "사회초년생", "사항", "상황", "새로운", "생각하기", "설계", "성공적인", "스마트한", "시작", "실거래", "실수", "실전",
  "실제", "실체", "실천", "안전한", "알아보는", "완성", "완전", "왜", "요소", "위한", "유형", "의미", "이끄는",
  "역할", "예시", "오해", "올바른", "우리", "움직이", "이란", "이유", "이해", "이해하기", "익히기", "있는", "작동", "장소",
  "적용", "원리", "정복", "정리", "정의", "정확히", "조건", "좋은", "주요", "주의사항", "주의점", "줄이기", "중요성",
  "지역별", "지침", "진실", "진짜", "차이", "체크", "체크리스트", "콘텐츠", "탄생", "따져보", "평가", "함께", "함정", "행동", "현장",
...
const PARTICLE_SUFFIXES = [
  "으로부터", "에서부터", "이라고", "이라는", "에서는", "으로는", "에서의", "하는", "해야", "하기", "적인",
  "으로", "에게", "까지", "부터", "처럼", "보다", "에는", "과는", "와는", "에서", "으로", "의", "과", "와", "을", "를",
  "은", "는", "이", "가", "에", "도", "만", "로",
];
...
export function extractContentKeywords(item: KeywordSourceItem): Set<string> {
  ...
  for (const rawToken of tokens) {
    if (STOP_WORDS.has(rawToken)) continue;
...
function normalizeKeywordToken(value: string): string | undefined {
  ...
  for (const suffix of PARTICLE_SUFFIXES) {
    ...
    if (token.length - suffix.length < 2) return undefined;
