...
사용자 브라우저
  └─ life-quiz Worker
      ├─ Astro SSR: 홈, 보관함, 글 상세, 복습, 채팅, 릴리즈 노트
      ├─ Static Assets: CSS, JS, WebP 이미지
      ├─ D1: 콘텐츠, 퀴즈, 복습 로그, 채팅 사용량, 릴리즈 캐시
      └─ Service Binding
...
4. 새 원본은 8초 이상 간격을 두고 처리하며, 최근 60초 호출이 12회면 다음 회차로 미룬다.
5. Gemini가 4개 `section`과 퀴즈를 만든다.
6. 각 section의 `summary`는 Quick Read 카드가 되고, `summary + details`는 Deep Read가 된다.
7. 코드가 카드 의미 중복과 Deep Read의 카드 포함 여부를 검사한 뒤 저장한다.
8. 역사·유머·사회성·생활상식은 4개 회차에 한 분야씩, 금융·주식·투자·부동산 기초 용어는 첫 3개 회차에 한 분야씩 분산 생성한다.
