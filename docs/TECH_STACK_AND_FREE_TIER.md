...
2. 금융위원회 공식 RSS에서 사회초년생과 직접 관련된 정책·금융 정보를 피드별로 교차 배치한 뒤 분야별 최대 2건, 회차당 최대 6건으로 고른다. YouTube 4개 주제는 주제별 후보 3개까지 받고 같은 회차에서 서울 자치구 공공데이터도 함께 수집한다.
...
9. 역사·유머·사회성·생활상식은 분야별 32개 커리큘럼의 한국어 위키백과 문서를 먼저 조회한다. 요약이 짧으면 문서 도입부로 보강하고, Cloudflare에서 직접 API가 차단되면 `api.wikimedia.org`의 공식 원문을 최대 256KB까지만 읽는다. 모든 직접 경로가 실패하면 `gemini-3.1-flash-lite` URL Context를 사용하고 `URL_RETRIEVAL_STATUS_SUCCESS` 메타데이터를 확인한 경우에만 저장한다.
10. 금융·주식·투자·부동산 기초 용어와 역사·유머·사회성·생활상식 후보는 매 회차 전체 후보를 회전 순서로 넣고, Gemini 예산과 중복 검사에 따라 순차 생성한다. 같은 분야의 최근 제목도 프롬프트에 전달해 일반적인 제목 반복을 줄인다.
...
- YouTube `search.list`는 현재 공식 문서상 하루 100회 기본 버킷이며 각 호출은 1 quota를 사용한다. 현재는 4개 주제를 회차마다 1회씩 검색하므로 하루 16회, 기본 버킷의 약 16%를 사용한다. 각 검색은 최대 3개 후보를 받아 이미 수집된 최신 영상 때문에 전체 주제가 멈추는 문제를 줄인다. [quota cost](https://developers.google.com/youtube/v3/determine_quota_cost)
...
- 회차당 YouTube 4개 주제별 후보 3개 수집과 AI 커리큘럼 후보 회전 생성
