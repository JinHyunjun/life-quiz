...
- 콘텐츠 생성은 예약 배치에서 수행하고 원문 URL 또는 학습판별 고유 수집 키가 중복되면 Gemini 호출 전에 제외한다.
...
| YouTube Data API v3 | 영상 메타데이터 | 8개 초보자 주제를 회차마다 2개씩 최근 180일 범위에서 검색하고 주제별 후보 5개의 제목, 설명, 링크만 사용 |
...
2. 금융위원회 공식 RSS에서 사회초년생과 직접 관련된 정책·금융 정보를 피드별로 교차 배치한 뒤 분야별 최대 2건, 회차당 최대 6건으로 고른다. YouTube는 8개 주제 중 2개씩 교대하며 최근 180일의 주제별 후보 5개까지 받고, 같은 회차에서 서울 자치구 4곳의 공공데이터도 함께 수집한다.
3. 뉴스는 키워드 관련성 검사와 분야 균형 선별, 공공데이터와 YouTube는 회차별 편집 계획으로 카테고리와 설명 관점을 Gemini 호출 전에 확정한다.
4. D1에서 KST 당일 분야별 공개 건수를 읽고 최소 1개에 못 미친 분야를 먼저 배치한다. 주거·서울살이 4개 등 분야별 상한에 도달한 후보는 Gemini 호출 전에 건너뛴다. 부동산은 전월세와 매매를 6시간마다 교대해 한 회차 한 건만 만든다.
...
11. 커리큘럼이 한 바퀴 돌면 출처 URL은 유지하되 발행 중복 키에 학습판 번호를 붙인다. 핵심 원리, 초보자 오해, 행동 체크리스트, 다시 설명하기의 네 관점을 교대하고 최근 제목을 함께 전달해 같은 내용과 표현의 반복을 줄인다.
12. 금융·주식·투자·부동산 기초 용어는 첫 세 회차에 하나씩 배정한다. 출처형 상식은 현재 회차의 새 분야뿐 아니라 앞 시간대 분야도 다시 후보에 올려 일시 실패를 같은 날 회복하며, 이미 성공한 학습판은 원격 조회와 Gemini 호출 전에 건너뛴다.
13. 회차 결과는 `ingestion_runs`에 pending, created, skipped, deferred, failed와 수집원별 success·empty·error 및 후보 수를 남긴다. Cron 자체가 실패해도 error row를 남겨 다음 점검에서 원인을 볼 수 있게 한다.
14. 검수에서 숨김 처리된 콘텐츠는 홈, 보관함, 상세, 채팅 근거, 5분 학습과 복습 큐에서 모두 제외한다.
...
- YouTube `search.list`는 현재 공식 문서상 하루 100회 상당의 기본 버킷이다. 현재는 8개 주제를 하루에 1회씩 검색하므로 하루 8회, 기본 버킷의 약 8%를 사용한다. 검색당 후보 수를 3개에서 5개로 늘려도 `search.list` 요청 횟수는 같으며, 최근 180일 범위에서 새 원문을 찾는다. [quota cost](https://developers.google.com/youtube/v3/determine_quota_cost)
- data.go.kr 호출 한도는 API와 승인받은 활용 신청별로 다르다. 포털의 각 활용 신청 화면을 기준으로 확인해야 하며, 코드에는 전역 무제한을 가정하지 않는다. 현재는 회차당 실거래가 4회와 생활폐기물 4회, 하루 약 32회 요청한다.
...
정상 응답 기준 배치가 만드는 Gemini 호출은 하루 최대 48회다. `GEMINI_INGEST_MAX_ITEMS=12`가 성공·실패를 합친 회차당 생성 시도를 제한하고 하루 4회 실행되기 때문이다. 429·5xx 일시 오류에는 한 번만 재시도하므로 장애 시 요청이 일부 늘 수 있지만, 재시도도 배치와 채팅이 함께 쓰는 12 RPM과 하루 400회 회로 차단기를 통과한다. 실제 호출은 원문·학습판 중복 제거와 빈 공공데이터 때문에 더 적다. 관리자 화면은 최근 14일 호출량과 콘텐츠 생성·채팅 목적별 오늘 사용량을 보여 준다.
...
- 서울 25개 자치구 순환 수집
- 회차당 서울 4개 자치구의 전월세·매매 교대 브리핑 1건과 생활정보 비교 브리핑, 분야 균형 뉴스 선별
- 회차당 YouTube 2개 주제, 출처형 AI 상식의 누적 재검사로 하루 안에 실패 분야를 복구하는 분산 수집
