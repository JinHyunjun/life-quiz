...
| 인증 | better-auth 1.6.21 | MIT | 로그인 우선 진입, 이메일·비밀번호 로그인, 30일 세션, D1 rate limit, 익명 학습 기록의 계정 병합. 별도 서명 없는 30일 게스트 쿠키는 접근 선택만 기억하고 사용자 신원을 증명하지 않음 |
...
| 테이블 | 역할 |
|---|---|
| `users` | 익명 브라우저 ID와 인증 계정 ID를 학습 기록에 연결하는 도메인 사용자 |
| `auth_user`, `auth_session`, `auth_account`, `auth_verification`, `auth_rate_limit` | better-auth 계정·비밀번호 해시·30일 세션·검증 토큰·인증 요청 제한. 학습 데이터 테이블과 분리 |
...
5. 일반 상식·용어 과정은 날짜별 후보를 여러 개 준비하고, 서울살이·권리·디지털 안전·건강은 서울시·고용노동부·KISA·질병관리청 공식 문서를 대체 후보로 사용한다. 먼저 성공한 한 건만 발행해 같은 날 과잉 생성을 막는다.
6. 뉴스·영상은 저장된 원문 URL, 공공데이터는 실제 응답 내용의 SHA-256 지문이 같으면 Gemini 호출 전에 건너뛴다. 같은 API 조회 URL이라도 거래 내역이 바뀌면 새 지문으로 갱신한다.
7. 새 원본은 8초 이상 간격을 두고 처리하며, 최근 60초 호출이 12회이거나 성공·실패를 합친 회차 생성 시도가 12번에 도달하면 나머지를 다음 회차로 미룬다.
8. Gemini가 4개 `section`, 상황형 퀴즈, 정답을 구분하는 해설을 만든다.
9. 각 section의 `summary`는 Quick Read 카드가 되고, `summary + details`는 같은 소제목의 Deep Read가 된다. 마지막 section은 확인 행동을 맡되 상세 화면 아래에서 같은 문장을 다시 노출하지 않는다.
10. 뉴스·용어·흥미 상식 모두 네 카드의 제목과 본문 의미 중복을 엄격히 검사한다. 3번 실제 사례와 4번 확인 행동도 같은 내용을 표현만 바꿔 반복하면 저장하지 않으며, 모든 유형에서 Deep Read의 카드 포함 여부를 함께 검사한다.
...
| D1 쓰기 | 100,000 rows/일 | 콘텐츠, 계정·세션, 일일 학습, 비공개 문의, 사용자 피드백, 복습 로그, 채팅·인증 제한 기록이 공유 |
...
2026-08-11 KST에 원격 D1을 `wrangler d1 info`와 검수 쿼리로 확인한 결과다. 당일 값은 18시 최종 수집 전 중간 수치다.

| 항목 | 현재 값 | 무료 한도 대비 |
|---|---:|---:|
| D1 크기 | 4.75MiB | 계정 저장소 5GB의 약 0.10%, DB당 500MB의 약 0.95% |
| 최근 24시간 rows read | 23,155 | 5,000,000의 약 0.46% |
| 최근 24시간 rows written | 216 | 100,000의 0.22% |
| 콘텐츠 / 퀴즈 | 796 / 796 | 텍스트 중심이라 저장 여유가 큼 |
| 공개 / 숨김 콘텐츠 | 755 / 41 | 숨김 콘텐츠는 모든 노출면에서 제외 |
| 학습 목록 / 복습 로그 / 5분 학습 카드 | 18 / 1 / 20 | 개인화 데이터를 포함해도 매우 작음 |
| 관심 분야 / 저장한 상식 | 0 / 0 | 첫 방문 관심 설정을 출시하기 전 기준선이며 이후 퍼널에서 추적 |
| 최근 공개 발행량 | 오늘 진행 중 9개·7분야, 직전 6일 9~16개·6~11분야/일 | 일일 양과 분야 모두 아직 최종 SLO 검증이 필요 |
| Cron | Life Quiz 1개 | Soccer 1개와 합산해 계정 2/5 사용 |
| YouTube 검색 | 8회/일 | 100회의 8% |
| 오늘 Gemini 요청 | 10회 | 내부 일일 예산 400회의 2.5% |
...
관심 분야, 저장한 상식, 학습 리포트, 로그인 우선 진입과 비공개 문의함은 기존 앱 요청과 D1 쿼리만 사용한다. 가입·로그인·문의 접수는 계정·세션·rate-limit·문의 쓰기를 조금 늘리지만 새 유료 서비스, Gemini 호출, 외부 API 호출은 만들지 않는다. 만료 인증 데이터와 해결·종료 후 180일이 지난 문의 정리도 기존 수집 Cron에 묶었으므로 Trigger 수는 그대로다.
...
2. **개인정보 최소화**: 이메일은 인증 또는 문의 답변에만 사용하고 익명 행동 이벤트와 분리한다. 문의에는 비밀번호·인증번호·계좌번호를 받지 않으며 원 IP는 저장하지 않는다. 비밀번호는 better-auth 기본 단방향 해시로 저장하고 개인정보 처리방침·이용약관을 계정 생성 전에 연결한다.
3. **SSR 요청 절감**: 조회가 많은 글 상세는 필요할 때 캐시하거나 정적 생성해 Worker와 D1 읽기를 줄인다.
4. **D1 인덱스 유지**: 현재 `created_at`, `(category, created_at)`, `(moderation_status, category, created_at)`, `(user_id, due)`, `(user_id, kst_date)`, `(status, created_at)`, `(created_at, event_name, visitor_id)` 인덱스를 유지하고 새 필터는 실행 계획을 확인한다.
