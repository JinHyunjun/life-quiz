...
| Wikimedia REST API | 일반 상식의 외부 근거 | 한국어 위키백과 요약을 하루 네 분야에 하나씩 조회하고 원문 URL과 CC BY-SA 표시 저장 |
...
2. RSS 후보에서 사회초년생의 금융·투자·주거·소비 생활과 직접 관련된 기사만 최대 3건 고르고, YouTube 2건과 해당 회차의 서울 자치구 공공데이터를 수집한다.
3. 뉴스는 키워드 관련성 검사, 공공데이터와 YouTube는 회차별 편집 계획으로 카테고리와 설명 관점을 Gemini 호출 전에 확정한다.
4. 저장된 source URL과 같으면 Gemini 호출 전에 건너뛴다.
5. 새 원본은 8초 이상 간격을 두고 처리하며, 최근 60초 호출이 12회면 다음 회차로 미룬다.
...
8. 코드가 카드 의미 중복과 Deep Read의 카드 포함 여부를 검사한 뒤 저장한다.
9. 역사·유머·사회성·생활상식은 회차별 커리큘럼의 한국어 위키백과 문서를 먼저 조회한 뒤, 그 문서 안의 사실만으로 생성하고 실제 원문 URL을 저장한다.
10. 금융·주식·투자·부동산 기초 용어는 첫 3개 회차에 한 분야씩 분산 생성한다.
...
- 서비스 내부에서는 D1의 60초 슬라이딩 윈도우로 배치와 채팅을 합산해 12회까지만 허용한다. 현재 AI Studio의 15 RPM 대비 3회의 여유를 남기며, 초과 요청은 Gemini에 보내지 않고 429 또는 다음 Cron 회차로 처리한다.
...
2026-06-29 원격 D1을 `wrangler d1 info`와 검수 쿼리로 확인한 결과다.

| 항목 | 현재 값 | 무료 한도 대비 |
|---|---:|---:|
| D1 크기 | 608KB | 5GB의 약 0.012% 미만 |
| 최근 24시간 rows read | 23,074 | 5,000,000의 약 0.46% |
| 최근 24시간 rows written | 579 | 100,000의 약 0.58% |
| 콘텐츠 / 퀴즈 | 111 / 111 | 매우 작음 |
| 공개 / 검수 대기 콘텐츠 | 70 / 41 | 기존 AI 상식 14개를 출처 기반으로 재작성해 복구, 미검증 29개와 주제 부적합 12개는 숨김 보존 |
...
배치가 만들 수 있는 Gemini 호출의 이론상 상한은 하루 약 39회다. 고정 생성 7회(트리비아 4 + 용어 3)에 새 RSS·YouTube·공공데이터가 모두 중복 없이 들어오는 32회를 더한 값이며 실제로는 URL 중복 제거 때문에 더 적다. 배치와 채팅 모두 공용 12 RPM 회로 차단기를 통과하지만 서비스 전체 RPD 상한은 아직 별도로 두지 않았다.
