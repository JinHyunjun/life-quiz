...
2026-07-31 KST에 원격 D1을 `wrangler d1 info`와 검수 쿼리로 확인한 결과다.

| 항목 | 현재 값 | 무료 한도 대비 |
|---|---:|---:|
| D1 크기 | 3.96MB | 계정 저장소 5GB의 약 0.08%, DB당 500MB의 약 0.79% |
| 최근 24시간 rows read | 46,368 | 5,000,000의 약 0.93% |
| 최근 24시간 rows written | 198 | 100,000의 약 0.20% |
| 콘텐츠 / 퀴즈 | 647 / 647 | 텍스트 중심이라 저장 여유가 큼 |
| 공개 / 숨김 콘텐츠 | 606 / 41 | 숨김 콘텐츠는 모든 노출면에서 제외 |
| 학습 목록 / 복습 로그 / 5분 학습 카드 | 10 / 1 / 10 | 개인화 데이터를 포함해도 매우 작음 |
| 관심 분야 / 저장한 상식 | 0 / 0 | 2026-07-31 출시 직후 기준이며 운영 대시보드에서 채택 추적 |
| 최근 공개 발행량 | 오늘 진행 중 10개, 직전 6일 11~15개/일 | 수집 회차와 분야 상한 안에서 일일 목표를 대체로 유지 |
...
정상 응답 기준 배치가 만드는 Gemini 호출은 하루 최대 48회다. `GEMINI_INGEST_MAX_ITEMS=12`가 성공·실패를 합친 회차당 생성 시도를 제한하고 하루 4회 실행되기 때문이다. 429·5xx 일시 오류에는 한 번만 재시도하므로 장애 시 요청이 일부 늘 수 있지만, 재시도도 배치와 채팅이 함께 쓰는 12 RPM 회로 차단기를 통과한다. 실제 호출은 URL 중복 제거, 이미 수집된 Wikipedia URL, 빈 공공데이터 때문에 더 적으며 서비스 전체 RPD 상한은 아직 별도로 두지 않았다.
