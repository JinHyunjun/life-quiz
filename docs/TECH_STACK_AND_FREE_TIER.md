...
| YouTube Data API v3 | 영상 메타데이터 | 8개 초보자 주제를 회차마다 모두 검색하고 자막·본문 없이 제목, 설명, 링크만 사용 |
...
2. 금융위원회 공식 RSS에서 사회초년생과 직접 관련된 정책·금융 정보를 피드별로 교차 배치한 뒤 분야별 최대 2건, 회차당 최대 6건으로 고른다. YouTube 8개 주제는 주제별 후보 3개까지 받고 같은 회차에서 서울 자치구 4곳의 공공데이터도 함께 수집한다.
...
10. 금융·주식·투자·부동산 기초 용어와 8개 출처형 상식 후보는 매 회차 전체 후보를 분야 교차 순서로 넣고, Gemini 예산과 중복 검사에 따라 순차 생성한다. 같은 분야의 최근 제목도 프롬프트에 전달해 일반적인 제목 반복을 줄인다.
...
- YouTube `search.list`는 현재 공식 문서상 하루 100회 상당의 기본 버킷이다. 현재는 8개 주제를 회차마다 1회씩 검색하므로 하루 32회, 기본 버킷의 약 32%를 사용한다. 각 검색은 최대 3개 후보를 받아 이미 수집된 최신 영상 때문에 전체 주제가 멈추는 문제를 줄인다. [quota cost](https://developers.google.com/youtube/v3/determine_quota_cost)
...
- 회차당 YouTube 8개 주제별 후보 3개 수집과 8개 출처형 AI 커리큘럼 후보 회전 생성
