...
이 문서는 현재 저장소와 실제 Cloudflare 설정을 기준으로 작성한 기술 명세다. 무료 티어 수치는 각 서비스의 공식 문서와 가장 최근 원격 사용량 점검을 기준으로 정리했으며, 제공사 정책에 따라 바뀔 수 있다.
...
사용자 브라우저
  └─ life-quiz Worker
      ├─ Astro SSR·정적 페이지: 홈, 시작 코스, 보관함, 글 상세, 5분 학습, 내 학습, 복습, 채팅, 릴리즈 노트
      ├─ Static Assets: CSS, JS, WebP 이미지
      ├─ D1: 콘텐츠, 일일 학습 진행, 관심 분야, 저장한 상식, 사용자 피드백, 퀴즈, 복습 로그, 채팅 사용량, 릴리즈 캐시
      └─ Service Binding
          └─ life-quiz-ingest Worker
              ├─ Hono 내부 API
              ├─ KST 00/06/12/18시 통합 Cron
...
|---|---|---:|---|
| 프론트·SSR | Astro 7.0.3 | MIT | 페이지, 컴포넌트, API 라우트. 데이터 페이지는 SSR, 5분 학습·복습 셸은 정적 생성 |
...
| Cron Triggers | 예약 수집 | `0 3/6 * * *` 한 표현식으로 하루 4회, KST 00/06/12/18시 |
...
| data.go.kr | 아파트 매매·전월세, 생활폐기물 | 서울 25개 자치구 중 회차마다 4개 구를 조사하고 매매·전월세·생활정보 비교 브리핑 3건으로 통합 |
| YouTube Data API v3 | 영상 메타데이터 | 8개 초보자 주제를 회차마다 2개씩 나눠 검색하고 자막·본문 없이 제목, 설명, 링크만 사용 |
| RSS | 기사·정책 메타데이터 | Cloudflare에서 실제 응답을 검증한 금융위원회 보도자료·카드뉴스 RSS의 제목·요약·링크만 사용 |
...
| `sources` | 원본 유형, 고유 수집 키, 마지막 수집 시각. 뉴스·영상은 원문 URL을, 공공데이터는 조회 URL과 응답 지문을 결합해 중복 차단 |
...
| `daily_session_items` | 익명 사용자와 KST 날짜별 5장 구성, 순서, 복습·새 카드 구분, 완료 시각 |
...
1. Cron이 `0 3/6 * * *` 한 표현식으로 KST 기준 하루 네 번 실행된다.
2. 금융위원회 공식 RSS에서 사회초년생과 직접 관련된 정책·금융 정보를 피드별로 교차 배치한 뒤 분야별 최대 2건, 회차당 최대 6건으로 고른다. YouTube는 8개 주제 중 2개씩 교대하며 주제별 후보 3개까지 받고, 같은 회차에서 서울 자치구 4곳의 공공데이터도 함께 수집한다.
3. 뉴스는 키워드 관련성 검사와 분야 균형 선별, 공공데이터와 YouTube는 회차별 편집 계획으로 카테고리와 설명 관점을 Gemini 호출 전에 확정한다.
4. D1에서 KST 당일 분야별 공개 건수를 읽고 최소 1개에 못 미친 분야를 먼저 배치한다. 주거 6개, 서울살이 4개 등 분야별 상한에 도달한 후보는 Gemini 호출 전에 건너뛴다.
5. 뉴스·영상은 저장된 원문 URL, 공공데이터는 실제 응답 내용의 SHA-256 지문이 같으면 Gemini 호출 전에 건너뛴다. 같은 API 조회 URL이라도 거래 내역이 바뀌면 새 지문으로 갱신한다.
6. 새 원본은 8초 이상 간격을 두고 처리하며, 최근 60초 호출이 12회이거나 성공·실패를 합친 회차 생성 시도가 12번에 도달하면 나머지를 다음 회차로 미룬다.
7. Gemini가 4개 `section`, 상황형 퀴즈, 정답을 구분하는 해설을 만든다.
8. 각 section의 `summary`는 Quick Read 카드가 되고, `summary + details`는 같은 소제목의 Deep Read가 된다. 마지막 section은 확인 행동을 맡되 상세 화면 아래에서 같은 문장을 다시 노출하지 않는다.
9. 뉴스·용어·흥미 상식 모두 네 카드의 제목과 본문 의미 중복을 엄격히 검사한다. 3번 실제 사례와 4번 확인 행동도 같은 내용을 표현만 바꿔 반복하면 저장하지 않으며, 모든 유형에서 Deep Read의 카드 포함 여부를 함께 검사한다.
10. 역사·유머·사회성·생활상식은 분야별 32개, 직장·권리·디지털 안전·건강은 분야별 20개 커리큘럼의 한국어 위키백과 문서를 먼저 조회한다. 요약이 짧으면 문서 도입부로 보강하고, Cloudflare에서 직접 API가 차단되면 `api.wikimedia.org`의 공식 원문을 최대 256KB까지만 읽는다. 모든 직접 경로가 실패하면 `gemini-3.1-flash-lite` URL Context를 사용하고 `URL_RETRIEVAL_STATUS_SUCCESS` 메타데이터를 확인한 경우에만 저장한다.
11. 금융·주식·투자·부동산 기초 용어는 첫 세 회차에 하나씩, 8개 출처형 상식은 회차마다 2개씩 배정해 하루 네 회차가 전체 분야를 고르게 다룬다. 같은 분야의 최근 제목도 프롬프트에 전달해 일반적인 제목 반복을 줄인다.
12. 회차 결과는 `ingestion_runs`에 pending, created, skipped, deferred, failed와 수집원별 success·empty·error 및 후보 수를 남긴다. Cron 자체가 실패해도 error row를 남겨 다음 점검에서 원인을 볼 수 있게 한다.
13. 검수에서 숨김 처리된 콘텐츠는 홈, 보관함, 상세, 채팅 근거, 5분 학습과 복습 큐에서 모두 제외한다.
...
| 외부 subrequest | 호출당 50회 | YouTube·AI 상식을 회차별로 분산하고 생성 시도를 12번으로 제한해 실패가 이어져도 한 회차 요청 수가 폭증하지 않게 설계 |
| 내부 CF subrequest | 호출당 1,000회 | 채팅의 Service Binding 1회는 여유 있음 |
| Worker 개수 | 100개 | 현재 2개 사용 |
| Cron Trigger | 계정당 5개 | Life Quiz는 1개 표현식으로 하루 4회 실행. Soccer 1개와 합산해 계정 2/5 사용 |
...
| D1 쓰기 | 100,000 rows/일 | 콘텐츠, 일일 학습, 사용자 피드백, 복습 로그, 채팅 제한 기록이 공유 |
...
- 같은 D1 로그를 사용해 KST 00시부터 배치와 채팅을 합산한 요청이 하루 400회에 도달하면 외부 호출 전에 차단한다. 화면에서 확인한 500 RPD의 80%를 내부 상한으로 둔 값이며, Google 프로젝트 한도가 달라지면 수집·앱 Worker의 `GEMINI_DAILY_BUDGET`을 함께 조정해야 한다.
...
- YouTube `search.list`는 현재 공식 문서상 하루 100회 상당의 기본 버킷이다. 현재는 8개 주제를 하루에 1회씩 검색하므로 하루 8회, 기본 버킷의 약 8%를 사용한다. 각 검색은 최대 3개 후보를 받아 이미 수집된 최신 영상 때문에 전체 주제가 멈추는 문제를 줄인다. [quota cost](https://developers.google.com/youtube/v3/determine_quota_cost)
...
2026-08-01 KST에 원격 D1을 `wrangler d1 info`와 검수 쿼리로 확인한 결과다.

| 항목 | 현재 값 | 무료 한도 대비 |
|---|---:|---:|
| D1 크기 | 4.03MB | 계정 저장소 5GB의 약 0.08%, DB당 500MB의 약 0.81% |
| 최근 24시간 rows read | 22,775 | 5,000,000의 약 0.46% |
| 최근 24시간 rows written | 180 | 100,000의 0.18% |
| 콘텐츠 / 퀴즈 | 658 / 658 | 텍스트 중심이라 저장 여유가 큼 |
| 공개 / 숨김 콘텐츠 | 617 / 41 | 숨김 콘텐츠는 모든 노출면에서 제외 |
| 학습 목록 / 복습 로그 / 5분 학습 카드 | 10 / 1 / 10 | 개인화 데이터를 포함해도 매우 작음 |
| 관심 분야 / 저장한 상식 | 0 / 0 | 2026-07-31 출시 직후 기준이며 운영 대시보드에서 채택 추적 |
| 최근 공개 발행량 | 오늘 진행 중 9개, 직전 6일 11~15개/일 | 수집 회차와 분야 상한 안에서 일일 목표를 대체로 유지 |
| Cron | Life Quiz 1개 | Soccer 1개와 합산해 계정 2/5 사용 |
| YouTube 검색 | 8회/일 | 100회의 8% |
| 오늘 Gemini 요청 | 9회 | 내부 일일 예산 400회의 2.25% |
...
관심 분야, 저장한 상식과 학습 리포트는 기존 앱 요청과 D1 쿼리만 사용한다. 새 Cron, Gemini 호출, 외부 API 호출을 추가하지 않았으므로 P1 개인화 자체가 수집·AI 무료 한도를 더 소비하지는 않는다.

정상 응답 기준 배치가 만드는 Gemini 호출은 하루 최대 48회다. `GEMINI_INGEST_MAX_ITEMS=12`가 성공·실패를 합친 회차당 생성 시도를 제한하고 하루 4회 실행되기 때문이다. 429·5xx 일시 오류에는 한 번만 재시도하므로 장애 시 요청이 일부 늘 수 있지만, 재시도도 배치와 채팅이 함께 쓰는 12 RPM과 하루 400회 회로 차단기를 통과한다. 실제 호출은 URL 중복 제거, 이미 수집된 Wikipedia URL, 빈 공공데이터 때문에 더 적다. 관리자 화면은 최근 14일 호출량과 콘텐츠 생성·채팅 목적별 오늘 사용량을 보여 준다.
...
- 브라우저별 저장한 상식과 학습 리포트
- 브라우저별 오늘의 5분 학습과 새로고침 후 진행 복원
- 고정 사유형 콘텐츠 피드백과 소규모 관리자 QA 큐
- 제한된 로그인과 소규모 공개 베타
...
1. **Gemini 전역 예산 유지**: 12 RPM과 KST 하루 400회 공용 예산을 유지한다. 관리자 화면에서 70%부터 주의, 85%부터 실패 상태로 표시하고 400회에서는 배치와 채팅을 외부 호출 전에 차단한다.
...
4. **D1 인덱스 유지**: 현재 `created_at`, `(category, created_at)`, `(moderation_status, category, created_at)`, `(user_id, due)`, `(user_id, kst_date)`, `(status, created_at)` 인덱스를 유지하고 새 필터는 실행 계획을 확인한다.
5. **배치 호출 재사용**: 원본 URL 중복 검사와 공용 콘텐츠 생성 방식을 유지한다. 사용자별 요약을 새로 만들지 않는다.
6. **Cron 통합 유지**: 새 수집원은 기존 통합 Cron의 네 회차 안에 넣고 작업 실패를 소스별로 격리한다. 독립 Cron을 추가해야 할 때는 같은 Cloudflare 계정의 전체 Trigger 수를 먼저 확인한다.
...
- 서울 25개 자치구 순환 수집
- 회차당 서울 4개 자치구의 매매·전월세·생활정보 비교 브리핑과 분야 균형 뉴스 선별
- 회차당 YouTube 2개 주제와 출처형 AI 상식 2개 주제를 교대해 하루 동안 8개 분야 전체를 다루는 분산 수집
- D1 `ingestion_runs` 기반 수집 회차 진단 로그
- 키워드·날짜·12개 분야·출처별 아카이브와 페이지네이션
...
- 복습 최대 2장과 다양한 분야의 새 카드로 구성되는 D1 기반 오늘의 5분 학습
- 최대 5개 관심 분야를 반영하는 5분 학습 우선순위
- 연속 학습일·최근 7일 활동·누적 복습·기억 성공률·분야별 기억 현황을 보여주는 내 학습 페이지
- 글 상세 저장과 내 학습 보관함을 연결한 저장한 상식 기능
- 글 상세 고정 사유 피드백과 운영 대시보드 콘텐츠 QA 큐
- 관심 분야 설정 사용자·저장한 상식·완료된 5분 학습을 집계하는 운영 채택 지표
- 12개 분야 일일 최소·상한 발행 정책과 대시보드 목표 대비 현황
...
- 브라우저별 익명 복습·관심 분야·저장 목록의 계정 귀속과 마이그레이션
