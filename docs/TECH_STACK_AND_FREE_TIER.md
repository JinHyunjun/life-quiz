...
| `ingestion_runs` | 수집 회차별 생성·중복·지연·실패 건수와 실패 항목. 오늘 생성량 급감 원인을 원격 D1에서 추적 |
...
2. RSS 후보에서 사회초년생의 금융·투자·주거·소비 생활과 직접 관련된 기사만 분야별 최대 2건, 회차당 최대 6건으로 고르고, YouTube 4개 주제는 주제별 후보 3개까지 받아 중복 최신 영상만 반복 선택되는 문제를 줄인다. 같은 회차에서 서울 자치구 공공데이터도 함께 수집한다.
...
11. 회차 결과는 `ingestion_runs`에 저장해 pending, created, skipped, deferred, failed를 남긴다. Cron 자체가 실패해도 error row를 남겨 다음 점검에서 원인을 볼 수 있게 한다.
