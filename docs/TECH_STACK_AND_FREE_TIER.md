...
이 문서는 현재 저장소와 실제 Cloudflare 설정을 기준으로 작성한 기술 명세다. 무료 티어 수치는 각 서비스의 공식 문서를 2026-07-09~10에 다시 확인한 값이며, 제공사 정책에 따라 바뀔 수 있다.
...
4. 뉴스·영상은 저장된 원문 URL, 공공데이터는 실제 응답 내용의 SHA-256 지문이 같으면 Gemini 호출 전에 건너뛴다. 같은 API 조회 URL이라도 거래 내역이 바뀌면 새 지문으로 갱신한다.
5. 새 원본은 8초 이상 간격을 두고 처리하며, 최근 60초 호출이 12회이거나 성공·실패를 합친 회차 생성 시도가 12번에 도달하면 나머지를 다음 회차로 미룬다.
6. Gemini가 4개 `section`, 상황형 퀴즈, 정답을 구분하는 해설을 만든다.
7. 각 section의 `summary`는 Quick Read 카드가 되고, `summary + details`는 같은 소제목의 Deep Read가 된다. 마지막 section은 확인 행동을 맡되 상세 화면 아래에서 같은 문장을 다시 노출하지 않는다.
8. 뉴스·용어·흥미 상식 모두 네 카드의 제목과 본문 의미 중복을 엄격히 검사한다. 3번 실제 사례와 4번 확인 행동도 같은 내용을 표현만 바꿔 반복하면 저장하지 않으며, 모든 유형에서 Deep Read의 카드 포함 여부를 함께 검사한다.
9. 역사·유머·사회성·생활상식은 분야별 32개, 직장·권리·디지털 안전·건강은 분야별 20개 커리큘럼의 한국어 위키백과 문서를 먼저 조회한다. 요약이 짧으면 문서 도입부로 보강하고, Cloudflare에서 직접 API가 차단되면 `api.wikimedia.org`의 공식 원문을 최대 256KB까지만 읽는다. 모든 직접 경로가 실패하면 `gemini-3.1-flash-lite` URL Context를 사용하고 `URL_RETRIEVAL_STATUS_SUCCESS` 메타데이터를 확인한 경우에만 저장한다.
10. 금융·주식·투자·부동산 기초 용어는 첫 세 회차에 하나씩, 8개 출처형 상식은 회차마다 2개씩 배정해 하루 네 회차가 전체 분야를 고르게 다룬다. 같은 분야의 최근 제목도 프롬프트에 전달해 일반적인 제목 반복을 줄인다.
11. 회차 결과는 `ingestion_runs`에 pending, created, skipped, deferred, failed와 수집원별 success·empty·error 및 후보 수를 남긴다. Cron 자체가 실패해도 error row를 남겨 다음 점검에서 원인을 볼 수 있게 한다.
12. 검수에서 숨김 처리된 콘텐츠는 홈, 보관함, 상세, 채팅 근거, 복습 큐에서 모두 제외한다.
