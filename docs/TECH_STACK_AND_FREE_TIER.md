...
이 문서는 현재 저장소와 실제 Cloudflare 설정을 기준으로 작성한 기술 명세다. 무료 티어 수치는 각 서비스의 공식 문서를 2026-06-27에 다시 확인한 값이며, 제공사 정책에 따라 바뀔 수 있다.
...
사용자 브라우저
  └─ life-quiz Worker
      ├─ Astro SSR: 홈, 보관함, 글 상세, 복습, 채팅
      ├─ Static Assets: CSS, JS, WebP 이미지
      ├─ D1: 콘텐츠, 퀴즈, 복습 로그, 채팅 사용량
      └─ Service Binding
...
| data.go.kr | 아파트 매매·전월세, 생활폐기물 | 서울 25개 자치구를 6시간마다 순환 |
| YouTube Data API v3 | 영상 메타데이터 | 자막·본문 없이 제목, 설명, 링크만 사용 |
...
| `content_items` | 본문, 카드 4개, 형식, 분야, 출처, 생성 시각 |
| `quiz_items` | 콘텐츠별 4지선다 문제와 정답 |
...
2. RSS 3건, YouTube 2건, 해당 회차의 서울 자치구 공공데이터를 수집한다.
3. 저장된 source URL과 같으면 Gemini 호출 전에 건너뛴다.
4. 새 원본은 Gemini가 4개 `section`과 퀴즈를 만든다.
5. 각 section의 `summary`는 Quick Read 카드가 되고, `summary + details`는 Deep Read가 된다.
6. 코드가 카드 의미 중복과 Deep Read의 카드 포함 여부를 검사한 뒤 저장한다.
7. 역사·유머·사회성·생활상식은 회차당 1개씩, 금융·주식·투자·부동산 기초 용어는 하루 1개씩 생성한다.
...
- 현재 모델 `gemini-3.1-flash-lite`는 공식 가격표에 텍스트 입력·출력 무료 티어가 있다.
- RPM, TPM, RPD는 프로젝트 단위이며 고정 수치를 문서에 박아두기보다 Google AI Studio의 현재 한도를 확인해야 한다.
...
- YouTube `search.list`는 현재 공식 문서상 Search Queries 버킷에서 1 unit을 쓰며 하루 100회까지다. 현재 Cron당 1회, 하루 4회로 약 4%를 사용한다. [search.list](https://developers.google.com/youtube/v3/docs/search/list)
- data.go.kr 호출 한도는 API와 승인받은 활용 신청별로 다르다. 포털의 각 활용 신청 화면을 기준으로 확인해야 하며, 코드에는 전역 무제한을 가정하지 않는다. 현재는 하루 약 12회 요청한다.
- RSS는 별도 호출 요금이 없지만 저작권이 사라지는 것은 아니다. 제목·요약만 재구성하고 원문 링크를 유지한다.

## 8. 현재 사용량 스냅샷

2026-06-27 원격 D1을 `wrangler d1 info`로 확인한 결과다.

| 항목 | 현재 값 | 무료 한도 대비 |
|---|---:|---:|
| D1 크기 | 299KB | 5GB의 약 0.006% 미만 |
| 최근 24시간 rows read | 16,704 | 5,000,000의 약 0.34% |
| 최근 24시간 rows written | 644 | 100,000의 0.65% 미만 |
| 콘텐츠 / 퀴즈 | 55 / 55 | 매우 작음 |
| 복습 로그 | 1 | 매우 작음 |
| Cron | 4개 | 5개 중 80% |
| YouTube 검색 | 4회/일 | 100회의 4% |

Worker 일일 요청량과 Gemini의 실제 프로젝트별 한도 사용률은 저장소에서 조회할 수 없으므로 Cloudflare Dashboard와 Google AI Studio에서 별도로 확인해야 한다.

배치가 만들 수 있는 Gemini 호출의 이론상 상한은 하루 약 51회다. 고정 생성 19회(상식 16 + 용어 3)에 새 RSS·YouTube·공공데이터가 모두 중복 없이 들어오는 경우를 더한 값이며 실제로는 URL 중복 제거 때문에 더 적다. 반면 채팅은 사용자별 시간당 8회 제한만 있고 서비스 전체 일일 제한이 없어, 사용자가 늘면 Gemini가 가장 먼저 병목이 될 수 있다.
...
- 하루 네 번의 RSS·YouTube 메타데이터·공공데이터 수집
- 매일 금융·주식·투자·부동산 기초 4컷 가이드
- 수만 건 규모의 콘텐츠와 퀴즈 저장
...
| 무제한 AI 채팅 | 전역 한도 없이 무료 Gemini quota 소진 가능 | 일일 전역 예산과 회로 차단기 추가 |
...
1. **Gemini 전역 예산 추가**: 사용자별 시간당 8회 외에 서비스 전체 일일 호출 수를 D1에 기록하고 70~80%에서 채팅을 일시 제한한다.
...
4. **D1 인덱스 유지**: 현재 `created_at`, `(category, created_at)`, `(user_id, due)` 인덱스를 유지하고 새 필터는 실행 계획을 확인한다.
