...
이 문서는 현재 저장소와 실제 Cloudflare 설정을 기준으로 작성한 기술 명세다. 무료 티어 수치는 각 서비스의 공식 문서와 2026-07-27 원격 사용량을 기준으로 다시 확인했으며, 제공사 정책에 따라 바뀔 수 있다.
...
사용자 브라우저
  └─ life-quiz Worker
      ├─ Astro SSR·정적 페이지: 홈, 시작 코스, 보관함, 글 상세, 5분 학습, 복습, 채팅, 릴리즈 노트
      ├─ Static Assets: CSS, JS, WebP 이미지
      ├─ D1: 콘텐츠, 일일 학습 진행, 사용자 피드백, 퀴즈, 복습 로그, 채팅 사용량, 릴리즈 캐시
      └─ Service Binding
...
2026-07-27 KST에 원격 D1을 `wrangler d1 info`와 검수 쿼리로 확인한 결과다.

| 항목 | 현재 값 | 무료 한도 대비 |
|---|---:|---:|
| D1 크기 | 3.42MB | 계정 저장소 5GB의 약 0.07%, DB당 500MB의 약 0.68% |
| 최근 24시간 rows read | 52,928 | 5,000,000의 약 1.06% |
| 최근 24시간 rows written | 242 | 100,000의 약 0.24% |
| 콘텐츠 / 퀴즈 | 599 / 599 | 텍스트 중심이라 저장 여유가 큼 |
| 공개 / 숨김 콘텐츠 | 558 / 41 | 숨김 콘텐츠는 모든 노출면에서 제외 |
| 학습 목록 / 복습 로그 | 1 / 1 | 매우 작음 |
| 최근 공개 발행량 | 오늘 15개, 최근 6일 12~21개/일 | 수집 회차와 분야 상한을 적용해도 목표 12개/일 유지 가능 |
