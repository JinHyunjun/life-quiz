...
사용자 브라우저
  └─ life-quiz Worker
      ├─ Astro SSR·정적 페이지: 홈, 시작 코스, 보관함, 글 상세, 5분 학습, 내 학습, 복습, 채팅, 계정, 릴리즈 노트
      ├─ Static Assets: CSS, JS, WebP 이미지
      ├─ better-auth: 선택형 이메일 로그인과 30일 세션
      ├─ D1: 인증, 콘텐츠, 학습 진행, 관심 분야, 행동 이벤트, 피드백, 퀴즈, 복습 로그, 채팅 사용량, 릴리즈 캐시
      └─ Service Binding
...
| 인증 | better-auth 1.6.21 | MIT | 이메일·비밀번호 로그인, 30일 세션, D1 rate limit, 익명 학습 기록의 계정 병합 |
...
| D1 쓰기 | 100,000 rows/일 | 콘텐츠, 계정·세션, 일일 학습, 사용자 피드백, 복습 로그, 채팅·인증 제한 기록이 공유 |
...
관심 분야, 저장한 상식, 학습 리포트와 선택형 로그인은 기존 앱 요청과 D1 쿼리만 사용한다. 가입과 로그인은 계정·세션·rate-limit 쓰기를 조금 늘리지만 새 유료 서비스, Gemini 호출, 외부 API 호출은 만들지 않는다. 만료 인증 데이터 정리도 기존 수집 Cron에 묶었으므로 Trigger 수는 그대로다.
...
2. **개인정보 최소화**: 이메일은 인증에만 사용하고 익명 행동 이벤트와 분리한다. 비밀번호는 better-auth 기본 단방향 해시로 저장하며 개인정보 처리방침·이용약관을 계정 생성 전에 연결한다.
