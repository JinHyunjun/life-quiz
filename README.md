...
- 누적 콘텐츠의 반복 키워드를 크기로 비교하는 지식 지도와 키워드별 전체 콘텐츠 목록
...
- Gemini가 하나의 학습 섹션에서 함께 생성하고 의미 중복 검사를 통과한 Quick Read와 소제목형 Deep Read. 화면 크기별 실제 이동 위치만 안내하고 마지막 위치에서 다음 버튼을 명확히 비활성화
...
| --- | --- | --- |
| Astro | 웹페이지를 만드는 틀 | 로그인, 홈, 보관함, 상세, 5분 학습, 내 학습, 복습, 채팅, 문의, 릴리즈 노트 화면을 렌더링 |
