...
역사·유머·사회성·생활 상식은 각 분야 32개, 직장·권리·디지털 안전·건강은 각 분야 20개 주제를 순환하며 출처 링크와 최소 근거 문장을 유지합니다. 위키백과 API가 Cloudflare에서 차단되거나 요약이 짧으면 MediaWiki·Wikimedia 원문 경로를 제한된 길이로 읽어 보강합니다. 외부 원문을 끝내 확인하지 못하면 Gemini가 URL만 보고 내용을 추정하게 하지 않고 해당 항목을 실패로 기록합니다. 상식 유형을 포함한 모든 AI 콘텐츠는 네 카드의 제목과 본문에 동일한 의미 중복 검사를 적용합니다.
...
Service Binding의 대상이 먼저 존재해야 하므로 수집 Worker를 앱보다 먼저 배포합니다. 배치 수집은 KST 00시, 06시, 12시, 18시에 실행됩니다. Cloudflare Cron은 UTC 기준이고, 무료 계정의 Trigger 수를 아끼기 위해 설정에는 `0 3/6 * * *` 하나만 등록합니다. 각 배치는 서울 자치구 4곳을 묶은 매매·전월세·생활 비교 브리핑과 분야별 RSS 뉴스를 수집합니다. YouTube는 8개 주제 중 2개씩, 출처형 AI 상식은 8개 주제 중 2개씩 네 회차에 나눠 하루 동안 전체 주제를 다루며, 금융·투자·부동산 용어는 첫 세 회차에 하나씩 배정합니다. 처리 순서는 당일 최소 발행량이 부족한 분야부터 다시 정렬하고, 분야별 상한에 도달한 후보는 Gemini 호출 전에 건너뜁니다. 중복 확인 뒤 성공 여부와 관계없이 최대 12개 항목만 생성을 시도합니다. 뉴스·영상은 원문 URL, 공공데이터는 응답 내용의 SHA-256 지문으로 중복을 판정하므로 같은 조회 URL의 데이터가 갱신되면 새 콘텐츠가 될 수 있습니다. 실행 결과는 `ingestion_runs`에 수집원별 후보 수와 오류까지 저장합니다.
